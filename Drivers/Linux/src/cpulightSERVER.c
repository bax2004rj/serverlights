...
int delayTime = 1; // Delay in s for the lights
...
int main(int argc, char argv[]){
    ...
    if(serialpoint == -1){
        stopRunning = 1;
        printf("\033[31m[ FATAL ]\033[37m: Serial port not connected.\n");
    } else {
...
int interpretCommandLineArgs(int argc, char argv[]){//Read inputs from command line
    int option;
    while((option = getopt(argc,argv,"csv"))!=-1){
        switch (option)
        {
        case 'c':
